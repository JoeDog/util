...
public final class FileUtils {
  ...
  /**
   * Probes the file for its content type.
   * <p>
   * @param  String   The path to the file
   * @return String   It's content type, i.e., application/gzip
   */
  public static String type(String name) {
    String res = null;

    try {
      res = Files.probeContentType(Paths.get(name));
  ...
  public final static boolean isZipped(String name) {  
    String type = FileUtils.type(name);
    switch (type) {
   ...
   * @return boolean  true for Zip format; false if not
   */
  public final static boolean isText(String name) {  
    String type = FileUtils.type(name);
    switch (type) {
      case "text/plain":
        return true;
